...
// Demo mode detections (no backend required)
const demoDetections = [
    ...
    { id: 4, type: 'RemoteID Beacon', freq: '900 MHz', rssi: -55, droneId: 'EU-1234', timestamp: new Date(Date.now() - 60000), status: 'friendly', isNew: false },
    { id: 5, type: 'Transmission Vidéo', freq: '5.8 GHz', rssi: -38, droneId: 'DX-8821', timestamp: new Date(Date.now() - 180000), status: 'threat', isNew: false },
];
...
export default function App() {
    ...
    // Simulate live detections (demo mode - no backend)
    useEffect(() => {
        const interval = setInterval(() => {
            setDetections(prev => prev.map(d => ({
                ...d,
                isNew: (new Date() - d.timestamp) < 60000
            })));
        }, 5000);
    ...
    return (
        <div className="flex h-screen bg-slate-950 text-slate-200 font-sans overflow-hidden">
            {/* Demo Mode Banner */}
            <div className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-amber-600 to-amber-500 text-white text-center py-1 text-xs font-medium">
                🚀 Mode Démonstration — Données simulées
            </div>

            {/* Sidebar */}
            <aside className={`${sidebarCollapsed ? 'w-20' : 'w-64'} bg-gradient-to-b from-slate-900 to-slate-950 border-r border-slate-800/50 flex flex-col z-20 transition-all duration-300 relative mt-6`}>
                ...
                <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="absolute top-10 -right-3 w-6 h-6 bg-slate-800 border border-slate-700 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-700 transition-all z-30">
            ...
            {/* Main */}
            <div className="flex-1 flex flex-col min-w-0 mt-6">
                {/* Header */}
                <header className="h-16 bg-gradient-to-r from-slate-900 to-slate-800 border-b border-slate-800/50 flex items-center justify-between px-6">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="hidden md:flex items-center gap-3">
                            <StatusPill icon={<Activity size={14} />} label="Système" value="DÉMO" color="amber" />
                            <StatusPill icon={<Radio size={14} />} label="Antennes" value="2/2" color="blue" />
                            <StatusPill icon={<AlertTriangle size={14} />} label="Menaces" value="2" color="red" pulse />
                        </div>
                    </div>
